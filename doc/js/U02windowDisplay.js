...
document.body.addEventListener('click', (e) => {
    ...
    document.getElementById(`${matchSubWindow}-modal`).style.display = 'none';
    document.getElementById(`${matchSubWindow}-container`).style.display = 'none';
});
...
document.body.addEventListener('click', (e) => {
    ...
    const matchSubWindow = matchIdWithClass(clickedElement.id, subWindowClassNames);
    if (!matchSubWindow) return;
    console.log("matchSubWindow:" + matchSubWindow);
    document.getElementById(`${matchSubWindow}-modal`).style.display = 'flex';
    document.getElementById(`${matchSubWindow}-container`).style.display = 'flex';
});

function matchIdWithClass(clickedBtnId, windowClassNames) {
    for (const className of windowClassNames) {
        if (clickedBtnId.startsWith(className)) {
            return className;
        };
    };
    return "";
